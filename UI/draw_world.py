...
class WorldUI:
  ...
  def reset_world(self):
    ...
    # add the Map
    self.axes.add_patch(Rectangle(self.map.origin, self.map.length, self.map.height, edgecolor=self.map.line, facecolor=self.map.fill))
    ...
    # - black band lines
    # - label
    self.axes.add_patch(Rectangle(self.convL.origin, self.convL.length, self.convL.height, edgecolor=self.convL.line, facecolor=self.convL.fill))
    self.axes.add_patch(Rectangle((self.convL.origin[0]-1, self.convL.origin[1]-1), 1, 4, edgecolor='black', facecolor='black'))
    ...
    self.axes.add_patch(Rectangle(self.convH.origin, self.convH.length, self.convH.height, edgecolor=self.convH.line, facecolor=self.convH.fill))
    self.axes.add_patch(Rectangle((self.convH.origin[0]-1, self.convH.origin[1]-1), 1, 4, edgecolor='black', facecolor='black'))
    ...
    self.axes.add_patch(Rectangle(self.delivery.origin, self.delivery.length, self.delivery.height, edgecolor=self.delivery.line, facecolor=self.delivery.fill))
    self.axes.text(self.delivery.origin[0]-0.5, self.delivery.origin[1]-1, 'Delivery')

    # add the Chargin stations
    self.axes.add_patch(Rectangle(self.chargeC.origin, self.chargeC.length, self.chargeC.height, edgecolor=self.chargeC.line, facecolor=self.chargeC.fill))
    self.axes.text(self.chargeC.origin[0]-0.5, self.chargeC.origin[1]-1, 'Charge 1')
    self.axes.add_patch(Rectangle(self.chargeD.origin, self.chargeD.length, self.chargeD.height, edgecolor=self.chargeD.line, facecolor=self.chargeD.fill))
  ...
  def add_items(self, n_light, n_heavy):
    ...
    for i in range(n_light):
      ...
      self.axes.add_patch(Rectangle(self.itemL.origin, self.itemL.length, self.itemL.height, edgecolor=self.itemL.line, facecolor=self.itemL.fill))
    ...
    for i in range(n_heavy):
      ...
      self.axes.add_patch(Rectangle(self.itemH.origin, self.itemH.length, self.itemH.height, edgecolor=self.itemH.line, facecolor=self.itemH.fill))
