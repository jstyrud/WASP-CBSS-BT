...
def dot_tree(
    ...
    Args:
        ...
        static (optional): if true prints the tree with standard color code, otherwise a node is colored depending on its status
    ...
    """
    def get_node_attributes(node, static):
        ...
        if isinstance(node, composites.Selector):
            ...
            if not static:
                color_ = get_status_color(node)
                attributes = ('parallelogram', color_, 'black')
            else:
...
def render_dot_tree(root: behaviour.Behaviour,
    ...
    Args:
        ...
        static: if true prints the tree with standard color code, otherwise a node is colored depending on its status
    Example:
