...
def dot_tree(
    ...
    if 'Sequence' in root.name:
        (node_shape, node_colour, node_font_colour) = ('box', 'orange', 'black')
...
def render_dot_tree(root: behaviour.Behaviour,
    ...
    filetypes = {}
    if dot:
        filetypes["dot"] = graph.write
    if png:
        filetypes["png"] = graph.write_png
    if svg:
        filetypes["svg"] = graph.write_svg
    for extension, writer in filetypes.items():
