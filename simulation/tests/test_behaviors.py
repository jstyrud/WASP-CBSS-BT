...
def test_at_station():
    ...
    sm.state.robot_pos = simulation.get_pose(simulation.Stations.CHARGE1)
    behavior.initialise()
    assert behavior.update() == pt.common.Status.SUCCESS

    behavior, _ = behaviors.get_node_from_string("at station CHARGE2", sm)
    sm.state.robot_pos = simulation.get_pose(simulation.Stations.CHARGE2)
    behavior.initialise()
    assert behavior.update() == pt.common.Status.SUCCESS

    behavior, _ = behaviors.get_node_from_string("at station CONVEYOR_HEAVY", sm)
    sm.state.robot_pos = simulation.get_pose(simulation.Stations.CONVEYOR_HEAVY)
    behavior.initialise()
    assert behavior.update() == pt.common.Status.SUCCESS

    behavior, _ = behaviors.get_node_from_string("at station CONVEYOR_LIGHT", sm)
    sm.state.robot_pos = simulation.get_pose(simulation.Stations.CONVEYOR_LIGHT)
    behavior.initialise()
    assert behavior.update() == pt.common.Status.SUCCESS
    ...
    sm.state.robot_pos = simulation.get_pose(simulation.Stations.DELIVERY)
    assert behavior.update() == pt.common.Status.SUCCESS
...
def test_moveto():
    ...
    for _ in range(10):
        behavior.update()
    assert sm.state.robot_pos == simulation.get_pose(simulation.Stations.CHARGE1)
    ...
    for _ in range(10):
        behavior.update()
    assert sm.state.robot_pos == simulation.get_pose(simulation.Stations.CHARGE2)
    ...
    for _ in range(10):
        behavior.update()
    assert sm.state.robot_pos == simulation.get_pose(simulation.Stations.CONVEYOR_HEAVY)
    ...
    for _ in range(10):
        behavior.update()
    assert sm.state.robot_pos == simulation.get_pose(simulation.Stations.CONVEYOR_LIGHT)
    ...
    for _ in range(10):
        behavior.update()
    assert sm.state.robot_pos == simulation.get_pose(simulation.Stations.DELIVERY)
